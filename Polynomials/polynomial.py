...
class Polynomial():
    def __init__(self, coefficients: list, truncate: bool = False, modulo: int = 0) -> None:
        ...
        if truncate:
            while len(self._cf) > 1 and self._cf[-1] == 0:
                self._cf.pop(-1)
    ...
    def __gt__(self, other) -> bool:
        if not isinstance(other, Polynomial):
            raise ValueError('Must pass in a Polynomial object')
    ...
    def __add__(self, p) -> Polynomial:
        if isinstance(p, Polynomial):
            ...
            return self + Polynomial([p], modulo=self.mod, truncate=True)
    ...
    def __truediv__(self, p: Polynomial) -> Polynomial | Remainder:
        ...
        while slot <= len(dividend) - len(divisor):
            ...
            slot += 1
        return [Polynomial(reversed(quo), truncate=True, modulo=self.mod), Remainder(Polynomial(reversed(dividend), truncate=True, modulo=self.mod), p)]
    ...
    def __neg__(self):
        p = self.copy()
        for i in range(len(p._cf)):
            p._cf[i] = (-p._cf[i])
            if p.mod != 0:
                p._cf[i] = p._cf[i] % p.mod
        return p
